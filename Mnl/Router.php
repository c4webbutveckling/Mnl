...
class Mnl_Router
{
	...
	function run()
	{
		...
		try {
		    if(is_readable(APPLICATION_PATH.'/'.Mnl_Registry::getInstance()->defaultControllerPath.'/'.$controller.'.php')) {
			    require_once(APPLICATION_PATH.'/'.Mnl_Registry::getInstance()->defaultControllerPath.'/'.$controller.'.php');
		    } else {
			    throw(new Mnl_Exception("Could not find controller: ".$controller));
		    }
